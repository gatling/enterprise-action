...
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.overrideLoadGeneratorsInputValidation = exports.configKeysInputValidation = exports.jsonValidation = exports.uuidValidation = exports.requiredBooleanValidation = exports.readConfig = void 0;
const core = __importStar(__nccwpck_require__(2186));
const idonttrustlikethat_1 = __nccwpck_require__(8505);
...
const readConfig = () => {
    ...
    (0, log_1.logDebug)("Parsed configuration: " + JSON.stringify({ api: { ...config.api, apiToken: "*****" }, run: config.run }));
    return config;
};
exports.readConfig = readConfig;
const getGatlingEnterpriseUrlConfig = () => getValidatedInput("gatling_enterprise_url", requiredInputValidation, "gatling_enterprise_url is required");
const getFailActionOnRunFailureConfig = () => getValidatedInput("fail_action_on_run_failure", exports.requiredBooleanValidation, "fail_action_on_run_failure is required");
const getWaitForRunEnd = () => getValidatedInput("wait_for_run_end", exports.requiredBooleanValidation, "wait_for_run_end is required");
const getApiConfig = (gatlingEnterpriseUrl) => {
    const apiToken = getValidatedInput("api_token", requiredInputValidation, "api_token is required", "GATLING_ENTERPRISE_API_TOKEN");
    return {
...
const getRunConfig = () => {
    const simulationId = getValidatedInput("simulation_id", exports.uuidValidation, "simulation_id must be a valid UUID");
    const extraSystemProperties = getValidatedInput("extra_system_properties", exports.configKeysInputValidation, "extra_system_properties must be a JSON object and only contain string, number, and boolean values (or omitted entirely)");
    const extraEnvironmentVariables = getValidatedInput("extra_environment_variables", exports.configKeysInputValidation, "extra_environment_variables must be a JSON object and only contain string, number, and boolean values (or omitted entirely)");
    const overrideLoadGenerators = getValidatedInput("override_load_generators", exports.overrideLoadGeneratorsInputValidation, "override_load_generators must be a valid configuration for overriding load generators");
    return {
        simulationId,
...
const requiredInputValidation = idonttrustlikethat_1.string.filter((str) => str !== "");
const optionalInputValidation = idonttrustlikethat_1.string.map((str) => (str === "" ? undefined : str));
exports.requiredBooleanValidation = requiredInputValidation.and((str) => {
...
exports.configKeysInputValidation = optionalInputValidation.then(configKeysValidation.optional());
...
exports.overrideLoadGeneratorsInputValidation = optionalInputValidation.then(overrideLoadGeneratorsValidation.optional());
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
const runMain_1 = __importDefault(__nccwpck_require__(5158));
const runCleanup_1 = __importDefault(__nccwpck_require__(5460));
const state_1 = __nccwpck_require__(9738);
const log_1 = __nccwpck_require__(8410);
const run = async () => {
    switch ((0, state_1.getPostStatusState)()) {
        case "post_noop":
            (0, log_1.log)("Post-execution cleanup: no cleanup is required.");
            break;
        case "post_cleanup":
            (0, log_1.log)("Post-execution cleanup: trying to stop simulation run if required.");
            await (0, runCleanup_1.default)();
            break;
        default:
            // Do not re-run the main action when launched again later
            (0, state_1.setPostStatusState)("post_noop");
            await (0, runMain_1.default)();
            break;
    }
};
run();
...
const duration_1 = __nccwpck_require__(2627);
const log_1 = __nccwpck_require__(8410);
const getAndLogMetricsSummary = async (client, runInfo) => {
...
const getMetricsSummary = async (client, runInfo) => {
    ...
    const duration = (0, duration_1.formatDuration)(runInfo.injectStart, currentTimestamp);
...
const logMetricsSummary = (summary) => {
    (0, log_1.logGroup)(`Time: ${summary.date}, ${summary.duration} elapsed\n`, (summary.nbUsers > 0 ? `Number of concurrent users: ${summary.nbUsers}\n` : "") +
...
const status_1 = __nccwpck_require__(1813);
const metrics_1 = __nccwpck_require__(4076);
const error_1 = __nccwpck_require__(9955);
const log_1 = __nccwpck_require__(8410);
const MAX_CONSECUTIVE_ERRORS = 5;
const waitForRunEnd = async (client, startedRun) => {
    let runInfo;
    let oldStatus = -1;
    ...
    do {
        try {
            ...
            runInfo.status !== oldStatus ? (0, log_1.log)(statusMsg) : (0, log_1.logDebug)(statusMsg);
            oldStatus = runInfo.status;
            if (runInfo.injectStart > 0) {
                await (0, metrics_1.getAndLogMetricsSummary)(client, runInfo);
            }
            ...
            if (consecutiveErrorsCount < MAX_CONSECUTIVE_ERRORS) {
                const msg = (0, error_1.formatErrorMessage)(error);
                (0, log_1.log)((0, log_1.yellow)(`Failed to retrieve current run information (attempt ${consecutiveErrorsCount}/${MAX_CONSECUTIVE_ERRORS}): ${msg}`));
            }
            else {
                throw error;
            }
...
const webAppUrl = (config, path) => path ? config.gatlingEnterpriseUrl + path : undefined;
...
const apiClient_1 = __nccwpck_require__(7533);
const state_1 = __nccwpck_require__(9738);
const error_1 = __nccwpck_require__(9955);
const log_1 = __nccwpck_require__(8410);
const run = async () => {
    try {
        ...
        const runId = (0, state_1.getRunIdState)();
        const stopped = await client.abortRun(runId);
        if (stopped) {
            (0, log_1.log)("Successfully stopped ongoing run");
        }
        else {
            (0, log_1.log)("There was no ongoing run to stop");
        }
    }
    catch (error) {
        (0, log_1.annotateError)("Failed attempt to clean up ongoing run, caused by: " + (0, error_1.formatErrorMessage)(error));
    }
};
exports["default"] = run;
...
const apiClient_1 = __nccwpck_require__(7533);
const status_1 = __nccwpck_require__(1813);
const start_1 = __nccwpck_require__(9459);
const ongoing_1 = __nccwpck_require__(2811);
const error_1 = __nccwpck_require__(9955);
const state_1 = __nccwpck_require__(9738);
const log_1 = __nccwpck_require__(8410);
const run = async () => {
    try {
        ...
        (0, state_1.setRunIdState)(startedRun.runId);
        (0, state_1.setPostStatusState)("post_cleanup"); // Run started, cleanup will be needed if interrupted now
        ...
        if (config.waitForRunEnd) {
            const finishedRun = await (0, ongoing_1.waitForRunEnd)(client, startedRun);
            (0, state_1.setPostStatusState)("post_noop"); // Run finished, no cleanup needed
            ...
            (0, state_1.setPostStatusState)("post_noop"); // Not waiting for run end, no cleanup needed
        }
    }
    catch (error) {
        (0, log_1.setFailed)((0, error_1.formatErrorMessage)(error));
    }
};
const logStart = (config, startedRun) => {
    (0, log_1.log)((0, log_1.bright)(`Started run ${startedRun.runId} for simulation ${config.run.simulationId}`));
    if (startedRun.reportsUrl) {
        (0, log_1.annotateNotice)(`Run reports will be available at ${startedRun.reportsUrl}`, "Gatling Enterprise reports");
    }
    if (startedRun.runsUrl) {
        (0, log_1.annotateNotice)(`Runs history is available at ${startedRun.runsUrl}`, "Gatling Enterprise runs history");
    }
    (0, log_1.log)("");
};
const logAssertionResults = (assertions) => {
    const assertionsCount = assertions.length;
    if (assertionsCount > 0) {
        (0, log_1.log)("");
        (0, log_1.log)((0, log_1.bright)("Assertion results:"));
        for (const assertion of assertions) {
            if (assertion.result) {
                (0, log_1.log)((0, log_1.green)(`> ${assertion.message} succeeded with value ${assertion.actualValue}`));
            }
            else {
                (0, log_1.log)((0, log_1.red)(`> ${assertion.message} failed with value ${assertion.actualValue}`));
            }
        }
        const assertionErrorsCount = assertions.reduce((acc, assertion) => (assertion.result ? acc : acc + 1), 0);
        if (assertionErrorsCount > 0) {
            (0, log_1.annotateError)(`${assertionErrorsCount} out of ${assertionsCount} assertions failed`, "Gatling assertion failures");
        }
        else {
            (0, log_1.annotateNotice)(`All ${assertionsCount} assertions passed successfully`, "Gatling assertions passed");
        }
    }
};
const logResult = (config, startedRun, finishedRun) => {
    (0, log_1.log)("");
    (0, log_1.log)((0, log_1.bright)("Simulation result:"));
    if ((0, status_1.isSuccessful)(finishedRun.statusCode)) {
        (0, log_1.log)((0, log_1.green)(`Run ${finishedRun.runId} finished with status ${finishedRun.statusName}`));
    }
    else {
        const errorMessage = `Run ${finishedRun.runId} failed with status ${finishedRun.statusName}`;
        (0, log_1.annotateError)(errorMessage);
    ...
    (0, log_1.log)("");
    if (startedRun.reportsUrl) {
        (0, log_1.log)((0, log_1.bright)(`See the run reports at ${startedRun.reportsUrl}`));
    }
    if (startedRun.runsUrl) {
        (0, log_1.log)((0, log_1.bright)(`See the runs history at ${startedRun.runsUrl}`));
    }
};
exports["default"] = run;
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.getPostStatusState = exports.setPostStatusState = exports.getRunIdState = exports.setRunIdState = exports.POST_STATUS_KEY = exports.RUN_ID_KEY = void 0;
const core = __importStar(__nccwpck_require__(2186));
exports.RUN_ID_KEY = "run_id";
exports.POST_STATUS_KEY = "post_status";
const setRunIdState = (runId) => core.saveState(exports.RUN_ID_KEY, runId);
exports.setRunIdState = setRunIdState;
const getRunIdState = () => core.getState(exports.RUN_ID_KEY);
exports.getRunIdState = getRunIdState;
const setPostStatusState = (status) => core.saveState(exports.POST_STATUS_KEY, status);
exports.setPostStatusState = setPostStatusState;
const getPostStatusState = () => {
    const rawValue = core.getState(exports.POST_STATUS_KEY);
    return rawValue === "post_noop" || rawValue === "post_cleanup" ? rawValue : undefined;
};
exports.getPostStatusState = getPostStatusState;
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.red = exports.yellow = exports.green = exports.bright = exports.setFailed = exports.annotateError = exports.annotateWarning = exports.annotateNotice = exports.logGroup = exports.log = exports.logDebug = void 0;
const core = __importStar(__nccwpck_require__(2186));
exports.logDebug = core.debug;
exports.log = core.info;
const logGroup = (title, message) => {
...
exports.annotateNotice = annotateNotice;
