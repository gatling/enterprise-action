...
const headers = (conf) => ({ Authorization: conf.apiToken });
...
const run = async () => {
    switch ((0, state_1.getPostStatusState)()) {
        case "post_noop":
            (0, log_1.logInfo)("Post-execution cleanup: no cleanup is required.");
            break;
        case "post_cleanup":
            (0, log_1.logInfo)("Post-execution cleanup: trying to stop simulation run if required.");
...
const logMetricsSummary = (summary) => {
    (0, log_1.logInfoGroup)(`Time: ${summary.date}, ${summary.duration} elapsed\n`, (summary.nbUsers > 0 ? `Number of concurrent users: ${summary.nbUsers}\n` : "") +
...
const waitForRunEnd = async (client, startedRun) => {
    ...
    do {
        try {
            ...
            runInfo.status !== oldStatus ? (0, log_1.logInfo)(statusMsg) : (0, log_1.logDebug)(statusMsg);
            ...
            if (errorCount < MAX_CONSECUTIVE_ERRORS) {
                ...
                (0, log_1.logWarning)(`Failed to retrieve current run information (attempt ${errorCount}/${MAX_CONSECUTIVE_ERRORS}): ${msg}`);
            }
            else {
                throw error;
            }
...
const startRun = async (client, config) => {
    ...
    (0, log_1.logInfo)(`Started run ${response.runId} for simulation ${config.run.simulationId}`);
    const reportsUrl = webAppUrl(config, response.reportsPath);
    if (reportsUrl) {
        (0, log_1.logInfo)(`Reports will be available at ${reportsUrl}`);
    }
    const runsUrl = webAppUrl(config, response.runsPath);
    if (runsUrl) {
        (0, log_1.logInfo)(`Runs history is available at ${runsUrl}`);
    }
...
const run = async () => {
    try {
        ...
        if (stopped) {
            (0, log_1.logInfo)("Successfully stopped ongoing run");
        }
        else {
            (0, log_1.logInfo)("There was no ongoing run to stop");
        }
    }
    catch (error) {
        (0, log_1.logError)("Failed attempt to clean up ongoing run, caused by: " + (0, error_1.formatErrorMessage)(error));
    }
};
exports["default"] = run;
...
const logResult = (config, finishedRun) => {
    if ((0, status_1.isSuccessful)(finishedRun.statusCode)) {
        (0, log_1.logSuccess)(`Run ${finishedRun.runId} finished with status ${finishedRun.statusName}`);
    }
    else {
        const errorMessage = `Run ${finishedRun.runId} failed with status ${finishedRun.statusName}`;
        if (config.failActionOnRunFailure) {
            (0, log_1.setFailed)(errorMessage);
        }
        else {
            (0, log_1.logError)(errorMessage);
        }
    }
    for (const assertion of finishedRun.assertions) {
        if (assertion.result) {
            (0, log_1.logSuccess)(`${assertion.message} succeeded with value ${assertion.actualValue}`);
        }
        else {
            (0, log_1.logError)(`${assertion.message} failed with value ${assertion.actualValue}`);
        }
    }
};
exports["default"] = run;
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.setFailed = exports.logError = exports.logWarning = exports.logSuccess = exports.logInfoGroup = exports.logInfo = exports.logDebug = void 0;
const core = __importStar(__nccwpck_require__(2186));
exports.logDebug = core.debug;
exports.logInfo = core.info;
const logInfoGroup = (title, message) => {
    core.startGroup(title);
    (0, exports.logInfo)(message);
    core.endGroup();
};
exports.logInfoGroup = logInfoGroup;
